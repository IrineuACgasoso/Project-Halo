...
class Crawler(InimigoBase):
    ...
    def iniciar_teleporte(self):
        ...
        while not posicao_valida and tentativas < 15:
            ...
            offset_x = math.cos(angulo) * distancia_ideal
            offset_y = math.sin(angulo) * distancia_ideal
    ...
    def update(self, delta_time, paredes=None):
        ...
        if self.estado == 'teleportando':
            if not self.particula_teleporte.alive():
                ...
                # Devolve a imagem correta pro Crawler reaparecer
                self.image = self.sprites[self.estado_animacao][self.frame_atual]
                self.rect = self.image.get_rect(center=self.posicao)
        ...
        if distancia > 0:
            ...
            elif distancia < distancia_ideal - 50:
                direcao_final -= direcao_alvo
